...
describe("Courses Spec", () => {
  ...
  it("Test adding and removing courses", function () {
    ...
    cy.get('[data-test="add-course-btn"]').click();
    cy.get(
      '[data-course-id="3"] > .flex-col > .justify-between > .flex-1 > [data-test="course-credits"]'
    ).should("have.text", "6 Credits");
    cy.get(
      '[data-test="prerequisites-section"] > [data-test="prerequisites-list"]'
    ).should("have.text", "PSY121");
    cy.get(
      '[data-course-id="3"] > :nth-child(2) > .space-y-4 > .justify-between > .gap-2'
    ).click();
    cy.get('[data-test="credits-completion"] > :nth-child(2)').should(
      "have.text",
      "9 / 9"
    ...
    cy.get('[data-test="add-course-btn"]').click();
    cy.get(
      '[data-course-id="4"] > :nth-child(2) > .space-y-4 > [data-test="prerequisites-section"]'
    ).should("have.text", "\r\n            Prerequisites PSY121, PSY213");
    cy.get(
      '[data-course-id="4"] > .mt-auto > [data-test="prerequisites-badge"]'
    ).should("have.text", "2 Prerequisites");
    cy.get(
      '[data-course-id="3"] > .mt-auto > [data-test="prerequisites-badge"]'
    ).should("have.text", "1 Prerequisite");
    cy.get('[data-course-id="4"] > .mt-auto').should(
      "have.text",
    ...
    cy.get(
      '[data-course-id="4"] > .flex-col > .justify-between > .flex-1 > .tracking-tight > [data-test="course-name"]'
    ...
    cy.get(
      '[data-course-id="4"] > :nth-child(2) > .space-y-4 > .justify-between > .gap-2 > [data-test="complete-checkbox"]'
    ).click();
    cy.get('[data-test="credits-completion"] > :nth-child(2)').should(
      "have.text",
      "15 / 15"
